(function(wHandle, wjQuery) {
    var CONNECTION_URL = "195.110.43.144:451", // Default Connection
        SKIN_URL = "./skinsof/"; // Skin Directory
...
var realX = nodeX;
        ...
        a.height = c;
        b.beginPath();
        var d = (realX + 7071) * c / 14142;
        var e = (realY + 7071) * c / 14142;
        b.arc(d, e, 5, 0, Math.PI * 2);
        b.closePath();
        b.fillStyle = '#FFFFFF';
        b.fill()
		},2400);
...
var a = document.getElementById("minimap");
        ...
        for (var j = 0; j < d; j++) {
            for (var i = 0; i < d; i++) {
                b.fillText(f[j] + (i + 1), (e * i) + (e / 2), (e * j) + (e / 2))
            }
        }
}
    ...
    function viewRange() {
        var ratio;
        ratio = Math.max(canvasHeight / 2280, canvasWidth / 2920);
        return ratio * zoom;
    }
    ...
    function drawGameScene() {
        ...
        if (0 != userScore) {
            ...
            var boxOpacity = .25;
            scoreText.setValue("Score: " + ~~(mass / 100));
            c = scoreText.render();
            w = c.width;
    ...
    Cell.prototype = {
            ...
            for (var ref = this, isvirus = this.isVirus ? 0 : (this.id / 1E3 + timestamp / 1E4) % (2 * Math.PI), j = 0; j < numpoints; ++j) {
                ...
                if (15 < this.size && null != qTree && 20 < this.size * viewZoom && 0 != this.id) {
                    ...
                    qTree.retrieve2(n - 5, q - 5, 10, 10, function(a) {
                        if (a.ref != ref && 0 > (n - a.x) * (n - a.x) + (q - a.y) * (q - a.y)) {
                            l = true;
                        }
                    });
                    if (points[j].x < leftPos || points[j].y < topPos || points[j].x > rightPos || points[j].y > bottomPos) {
                        l = true;
                    }
                    if (l) {
                        if (0 < pointsacc[j]) {
        ...
        drawOneCell: function(ctx) {
            if (this.shouldRender()) {
				...
				if(this.size<29&&this.id%8<4)b=false;
                if (b) {
                    ctx.beginPath();
                ...
                if (0 != this.id) {
                    var b = ~~this.y;
                    if ((showName || c) && this.name && this.nameCache && (null == e || -1 == knownNameDict_noDisp.indexOf(showSkin))) {
        ...
        render: function() {
            ...
            if (this._dirty) {
                ...
                var canvas = this._canvas,
                    ...
                    scale = this._scale,
                    fontsize = this._size,
                    font = "900 " + fontsize + 'px Roboto,Arial';
                ctx.font = font;
                var h = ~~(.2 * fontsize);
