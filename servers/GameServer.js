...
sIO.on = (function () {
    var messages = {};
    return function (message, handler, speedLimit) {
...
function GameServer(realmID,confile) {
    ...
    this.config = { // Border - Right: X increases, Down: Y increases (as of 2015-05-20)
		...
		virusSpliting: 1,
		virusExpMaxMass: 1000,
		virusWars: 0,
		virusWarsShootMass: 150,
		...
		snakerDish: 0,
		snakerPlayerSpeed: 30,
		snakerEjectSpeed: 80,
		snakerEjectMass: 3,
		snakerLossMass: 8,
		snakerDishMaxCells: 16,
...
GameServer.prototype.ban = function(ip) {
	var banlist = fs.readFileSync('banlist.txt').toString()
	if(banlist.split(ip+"<br>").length==1){
		fs.writeFileSync('banlist.txt', banlist+ip+"<br>");
	}
}
...
GameServer.prototype.start = function() {
    ...
    function connectionEstablished(ws) {
		...
		for(var i=0;i<this.clients.length;i++){
			if(ws._socket.remoteAddress==this.clients[i].playerTracker.ip){
				connections++
			}
		}
		if(connections>=this.config.playersOnIp){ws.close(); return;}
		sIO.on("connection",()=>{
        // Master server stuff
        if (ws && ws._socket && (this.config.masterIP == ws._socket.remoteAddress) && (!this.masterServer) && (this.config.useWithMaster)) {
            ws.gameServer = this;
		...
		var banlist = fs.readFileSync('banlist.txt').toString();
		for(var i=0;i<banlist.split("<br>").length;i++){
			if(banlist.split("<br>")[i]==ws._socket.remoteAddress){ws.close(); return;}
		}
        ws.remoteAddress = ws._socket.remoteAddress;
        ws.remotePort = ws._socket.remotePort;
		ws.token = ws.upgradeReq.headers['sec-websocket-key'];
...
GameServer.prototype.spawnFood = function() {
    ...
    if (this.config.HardCore==1) {
		f_size=Math.round(Math.random(this.config.maxFoodMass,this.config.minFoodMass)*this.config.maxFoodMass);
		var f = new Entity.Food(this.getNextNodeId(), null, this.getRandomPosition(), f_size);
		this.config.foodMass = f_size;
	}
...
GameServer.prototype.updateMoveEngine = function() {
    ...
    for (var i = 0; i < len; i++) {
        ...
        for (var j = 0; j < list.length ; j++) {
            ...
            // Consume effect
			if(this.config.DEATHMATCH==0||check.cellType!=2){
				check.onConsume(cell,this);
				if (check.cellType == 3) {
					if(this.config.CrazyFFA==1 && cell.mass<this.config.ejectMass && check.client!=client){this.removeNode(cell); if(check.client!=client&&this.config.DEATHMATCH==1&&client.cells.length==0){check.client.score++; client.score=0;}}
				}
				// Remove cell
...
GameServer.prototype.splitCells = function(client) {
    ...
    for (var i = 0; i < len; i++) {
        ...
        split.calcMergeTime(this.config.playerRecombineTime, this.config.MegaSplit);
...
GameServer.prototype.newCellVirused = function(client, parent, angle, mass, speed) {
    ...
    newCell.calcMergeTime(this.config.playerRecombineTime, this.config.MegaSplit);
...
GameServer.prototype.shootVirusWars = function(parent, angle) {
    var parentPos = {
        x: parent.position.x + ( (parent.getSize() + this.config.virusStartMass - 195) * Math.sin(angle) ),
        y: parent.position.y + ( (parent.getSize() + this.config.virusStartMass - 195) * Math.cos(angle) ),
    };

	parent.mass -= this.config.virusWarsShootMass;
	
    var newVirus = new Entity.Virus(this.getNextNodeId(), null, parentPos, this.config.virusStartMass);
    newVirus.setAngle(angle);
    newVirus.setMoveEngineData(200, 20);
	newVirus.addMassOnEat = false;
...
GameServer.prototype.shootMass = function(parent) {
 for (var i = 0; i < parent.cells.length; i++) {
	var cell = parent.cells[i];
	if(cell&&cell.mass<this.config.snakerLossMass+10){if(parent.cells.length>1){this.removeNode(cell);}; return;}
	var angle = -Math.atan2(parent.mouse.y - cell.position.y, parent.mouse.x - cell.position.x) - 90 * Math.PI / 180;
	var angle2 = -Math.atan2(parent.mouse.y - cell.position.y, parent.mouse.x - cell.position.x) + 90 * Math.PI / 180;
    var parentPos = {
        x: cell.position.x + ( (cell.getSize() + this.config.snakerEjectMass - 100) * Math.sin(angle) ),
        y: cell.position.y + ( (cell.getSize() + this.config.snakerEjectMass - 100) * Math.cos(angle) ),
    };

	cell.mass -= this.config.snakerLossMass;
	
     // Create cell
    var ejected = new Entity.EjectedMass(this.getNextNodeId(), null, parentPos, this.config.snakerEjectMass);
	ejected.isSnakingMass = true;
    ejected.setAngle(angle);
    ejected.setMoveEngineData(this.config.snakerEjectSpeed, 20);
	ejected.setColor(cell.getColor());
	ejected.client=parent;

	cell.setAngle(angle2);
	cell.setMoveEngineData(this.config.snakerPlayerSpeed, 20);
	
    this.addNode(ejected);
    this.setAsMovingNode(ejected);

	cell.calcMergeTime(this.config.playerRecombineTime, this.config.MegaSplit);
    ...
    for (var i = 0;i < len;i++) {
        ...
        switch (check.getType()) {
            ...
            case 0: // Players
                ...
                if (this.gameMode.haveTeams || this.config.DEATHMATCH == 1) {
                    if (!check.owner) { // Error check
                        continue;
                    }

                    if ((check.owner != cell.owner) && (check.owner.getTeam() == cell.owner.getTeam()) || (check.owner != cell.owner) && this.config.DEATHMATCH == 1) {
                        continue;
                    }
                }
                break;
