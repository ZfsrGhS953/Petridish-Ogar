...
glob_Bcolor=getCookie("backgroundColor_Glob_var");
glob_Scolor=getCookie("setkaColor_Glob_var");
show_chat_var=getCookie("show_chat_glob_var");
autorender_var=getCookie("autorender_glob_var");
if (!glob_Bcolor) {glob_Bcolor="#3aa025";}
if (!glob_Scolor) {glob_Scolor="#000000";}
if (!show_chat_var) {show_chat_var="1";}
if (!autorender_var) {autorender_var="true";}
var global_x=0;
var global_y=0;
var global_size=0;
var global_name="0";
...
GameLoop(function(fps){
    if(set) this.fps = 50;
    fps_box.value=Math.floor(fps);
}, 0);

function global_fps_set(){
setTimeout(function(){
global_fps=fps_box.value;
global_fps_set();
},5)
}
global_fps_set();
...
(function (wHandle, wjQuery) {
     ...
     *
     */
    var CONNECTION_URL = "185.118.164.79:81";
    /**
...
var SKIN_URL = "./skins/";
    ...
    function drawChatBoard() {
	    ...
	    if (chat_stop=="1") {return;}
		chatlog.style.padding="5px 15px 5px 15px";
        var mg = "<div>";
        mg += "<strong onclick='' style='color:" + color + "; text-shadow: black 1px 1px 1px; font-size: 115%;'>" + name+ ": "+ message +"</strong></div>";
		if (chatBoard[i + from].name.toLowerCase()=="greenadmin") {
		var mg = "<div style='background-Color: green; border-radius: 10px; border: 4px solid green; margin-top: 2px; margin-bottom: 2px;'>";
		mg += "<strong onclick='' style='color: lime; text-shadow: black 1px 1px 1px; font-size: 115%; padding: 5px 5px 5px 5px;'>" + name+ ": "+ message +"</strong></div>";
		}
		if (chatBoard[i + from].name.substr(0,5).toLowerCase()=="[vip]") {
		var mg = "<div>";
		mg += "<strong onclick='' style='color: red; text-shadow: black 1px 1px 1px; font-size: 115%; font-weight: 900;'>" + name+ ": "+ message +"</strong></div>";
		}
		$("#chatlog").append(mg);
        ...
        var from = len - 15;
        if (from < 0) from = 0;
            i=chatlog.children.length;
			addChat(chatBoard[i + from].name, chatBoard[i + from].message, chatBoard[i + from].color)
/*           var chatName = new UText(18, chatBoard[i + from].color);
            ...
            var chatText = new UText(18, chatBoard[i + from].color)
    ...
    function drawGameScene() {
		...
		userScoreIs = Math.floor(calcUserScore()/100);
        if (0 != userScore) {
            if (null == scoreText) {
                scoreText = new UText(24, "#FFFFFF");
            }
            if (playerCells[0]) {player_exist="true"; spectateBtn.disabled=true;} else {player_exist="false"; spectateBtn.disabled=false;}
            if (playerCells[0]) {global_name=playerCells[0].name;}
			if (playerCells[0]) {global_x=playerCells[0].ox;}
			if (playerCells[0]) {global_y=playerCells[0].oy;}
			if (playerCells[0]) {global_size=userScoreIs;}
			if (playerCells[0]) {playerCells[0].ss=global_ss;}
			if (playerCells[0]) {user=playerCells[0];}
			if (playerCells[0]) {if (playerCells[0].color=="#000000") {diemessage.style.opacity='1'; diemessage.style.pointerEvents='auto';}}
            scoreText.setValue('Рекорд: ' + ~~(userScore / 100) + ' | Сейчас: ' + Math.round(global_size) + " | FPS: " + global_fps + ' | x: ' + Math.round(global_x) + ' | y: ' + Math.round(global_y));
            c = scoreText.render();
            a = c.width;
            ctx.globalAlpha = .2;
    ...
    function drawGrid() {
        ctx.fillStyle = showDarkTheme ? glob_Bcolor : glob_Bcolor;
        ctx.fillRect(0, 0, canvasWidth, canvasHeight);
        ctx.save();
        ctx.strokeStyle = showDarkTheme ? glob_Scolor : glob_Scolor;
        ctx.globalAlpha = .2;
        ctx.scale(viewZoom, viewZoom);
        var a = canvasWidth / viewZoom,
            b = canvasHeight / viewZoom;
        if(a<canvasWidth*50&&b<canvasHeight*50){
    ...
    function drawLeaderBoard() {
        ...
        if (null != teamScores || 0 != leaderBoard.length)
            if (null != teamScores || showName) {
                ...
                var scaleFactor = Math.min(0.22*canvasHeight, Math.min(200, .3 * canvasWidth)) / 180;
    ...
    Cell.prototype = {
        ...
        drawOneCell: function (ctx) {
            if (this.shouldRender()) {
				...
				function load_clan_skin(skinArg){
					if (skinName==skinArg) {skinName="";}
					if (skinName.indexOf('[') != -1) {
					if (skinName.split("\[")[1].split("\]")[0]==skinArg) {
			...
			load_clan_skin("cv");
			load_clan_skin("☆pkb☆");
			load_clan_skin("illum");
			load_clan_skin("bat");
                if (this.name!="q" && this.name!="Q") {
			load_clan_skin("q");
                } else {
                skinName='q_ADMIN';
                }
			load_clan_skin("a");
			load_clan_skin("vst");
				if (this.name.substr(0,7)=="(18-25)") {skinName='18-25_A';}
				if (this.name.substr(0,7)=="[18-25]") {skinName='18-25_B';}
				if (this.name.substr(0,7)=="|18-25|") {skinName='18-25_C';}
				if (this.name.substr(0,5)=="[ƬψƬ]") {skinName='ƬψƬ';}
				if (this.name.substr(0,6)=="[USSR]") {skinName='ussr_clan';}
				if (this.name.substr(0,8)=="|ŦόþŖãκ|") {skinName='toprak_A';}
				if (this.name.substr(0,8)=="{ŦόþŖãκ}") {skinName='toprak_B';}
				if (this.name.substr(0,8)=="[ŦόþŖãκ]") {skinName='toprak_C';}
				if (this.name.substr(0,9)=="[Ŧόþ★Ŗãκ]") {skinName='toprak_D';}
				if (this.name.substr(0,5)=="[vip]") {skinName=this.name.substr(5);}
				if (this.name=="[ƬψƬ]☢Vi℘ βǿɤ✔") {skinName='[ƬψƬ]☢Vi℘ βǿɤ✔';}
				if (this.name=="[A]Adventypa") {skinName='adventypa';}
				if (this.name=="[VST]☢Vi℘ βǿɤ✔") {skinName='vst_ADMIN';}
				if (this.name=="[VST]MazaFacke") {skinName='vst_MAZAFASKE';}
				if (this.name.toLowerCase()=="greenbot+") {skinName='greenbot';}
				if (this.isVirus) {skinName="virus";}
                ...
                if (0 != this.id) {
                    ...
                    if ((showName || c) && this.name && this.nameCache && (null == e || -1 == knownNameDict_noDisp.indexOf(skinName))) {
                        ...
                        var rnchache = ncache.render(),
                            m = Math.floor(rnchache.width / viewZoom),
                            h = Math.floor(rnchache.height / viewZoom);
                        ctx.drawImage(rnchache, Math.floor(this.x) - Math.floor(m / 2), b - Math.floor(h / 2), m, h);
                    ...
                    if (showMass && (c || 0 == playerCells.length && (!this.isVirus || this.isAgitated) && 20 < this.size)) {
                        ...
                        c = this.sizeCache;
						c.setValue(Math.ceil(this.size * this.size / 100));
                        c.setSize(this.getNameSize() / 2 * viewZoom);
                        e = c.render();
                        m = Math.floor(e.width / viewZoom);
                        h = Math.floor(e.height / viewZoom);
                        ctx.drawImage(e, Math.floor(this.x) - Math.floor(m / 2), b - Math.floor(h / 2), m, h);
                    }
                }
                ctx.restore()
            }
