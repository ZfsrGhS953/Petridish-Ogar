...
function UpdateFragboard(playerTracker, leaderboard, leaderboardType) {
    this.playerTracker = playerTracker;
    this.leaderboard = leaderboard;
    this.leaderboardType = leaderboardType;
}

module.exports = UpdateFragboard;

UpdateFragboard.prototype.build = function (protocol) {
...
// UserText
UpdateFragboard.prototype.build48 = function (protocol) {
...
UpdateFragboard.prototype.build49 = function (protocol) {
    ...
    var writer = new BinaryWriter();
    writer.writeUInt8(0x31);                                // Packet ID
	writer.writeUInt32(this.leaderboard.length+this.playerTracker.gameServer.realStats.length);       // Number of elements
	for(var i=0;i<this.playerTracker.gameServer.realStats.length;i++){
        ...
        if (protocol <= 5)
            ...
            writer.writeStringZeroUtf8(this.playerTracker.gameServer.realStats[i]);
			}
    for (var i = 0; i < this.leaderboard.length; i++) {
        var item = this.leaderboard[i];
        if (item == null) return null;  // bad leaderboardm just don't send it
...
UpdateFragboard.prototype.build50 = function (protocol) {
    var writer = new BinaryWriter();
